#rm(list = ls())
#setwd("E:\\Desktop\\Supplementary_Data\\1.common DEGS\\GSE115018")
#if (!requireNamespace("BiocManager", quietly = TRUE))
#install.packages("BiocManager")

library(GEOquery)
gset = getGEO('GSE115018',destdir = '.',getGPL = F,
              AnnotGPL = T)
gset = gset[[1]]          
expr = exprs(gset)        
pdata = pData(gset)       
...
probe = read.table(file = 'GPL20115-26806.txt',
                   sep = '\t',
                   quote = '',         
                   comment.char = '#',    
                   header = T,
...
p <- boxplot(exprSet,outline=FALSE,las=2,col = 'blue',xaxt = 'n',ann = F,ylim=c(-6, 8))
title(main = list('GSE115018',cex = 2 ,font = 2),
...
par(pin=c(5, 3),family='serif',cex.axis=1.5)
p1 <- boxplot(normalized_expr,outline=FALSE,las=2,col = 'red',xaxt = 'n',ann = F,ylim=c(-6, 8))
title(main = list('GSE115018',cex = 2 ,font = 2),
      xlab = list('Sample list',cex = 1.5,font = 2),
      ylab = '',line = 0.7)
mtext('Expression value',side = 2,padj = -3,font = 2,cex = 1.5)
...
control = normalized_expr[,grep('paracancerous',group_list)]
...
GSE115018_diff=allDiff
GSE115018_diff=GSE115018_diff[order(GSE115018_diff$logFC),]
GSE115018_diff=rbind(Gene=colnames(GSE115018_diff),GSE115018_diff)
write.table(GSE115018_diff,file="GSE115018_diff.txt",sep="\t",quote=F,col.names=F)
...
ggplot(data= allDiff, aes(x = -log10(adj.P.Val), y = logFC, color = change)) +
  ...
  xlim(0,xMax) + 
  ylim(-yMax,yMax) +
  labs(title = 'GSE115018', x = '-log10(adj.P.Val)', y = 'logFC',  colour="black")
dev.off()

top100exp = exprSet1[rownames(diffSig)[1:100],]
#write.table(top100exp,file="top100exp.xls",sep="\t",quote=F)
#top100exp <- read.delim("top100exp.xls", row.names=1)
