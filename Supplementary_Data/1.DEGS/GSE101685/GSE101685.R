#rm(list = ls())
#setwd("E:\\Desktop\\Supplementary_Data\\1.common DEGS\\GSE101685")
#if (!requireNamespace("BiocManager", quietly = TRUE))
#install.packages("BiocManager")

library(GEOquery)
gset = getGEO('GSE101685',destdir = '.',getGPL = F,
              AnnotGPL = T)
gset = gset[[1]]          
expr = exprs(gset)        
pdata = pData(gset)       
...
p <- boxplot(exprSet,outline=FALSE,las=2,col = 'blue',xaxt = 'n',ann = F,ylim=c(1, 11))
title(main = list('GSE101685',cex = 2 ,font = 2),
...
par(pin=c(5, 3),family='serif',cex.axis=1.5)
p1 <- boxplot(normalized_expr,outline=FALSE,las=2,col = 'red',xaxt = 'n',ann = F,ylim=c(1, 11))
title(main = list('GSE101685',cex = 2 ,font = 2),
      xlab = list('Sample list',cex = 1.5,font = 2),
      ylab = '',line = 0.7)
mtext('Expression value',side = 2,padj = -3,font = 2,cex = 1.5)
...
#table(pdata$title)
control = normalized_expr[,grep('Normal liver',group_list)]
...
GSE101685_diff=allDiff
GSE101685_diff=GSE101685_diff[order(GSE101685_diff$logFC),]
GSE101685_diff=rbind(Gene=colnames(GSE101685_diff),GSE101685_diff)
write.table(GSE101685_diff,file="GSE101685_diff.txt",sep="\t",quote=F,col.names=F)
...
ggplot(data= allDiff, aes(x = -log10(adj.P.Val), y = logFC, color = change)) +
  ...
  xlim(0,xMax) + 
  ylim(-yMax,yMax) +
  labs(title = 'GSE101685', x = '-log10(adj.P.Val)', y = 'logFC',  colour="black")
dev.off()

top100exp = exprSet1[rownames(diffSig)[1:100],]
write.table(top100exp,file="top100exp.xls",sep="\t",quote=F)
#top100exp <- read.delim("E:/GEO_datasets/GSE101685/top100exp.xls", row.names=1)
